...
The rise in popularity and use of HEIF/HEIC(High Efficency Image Format) means your project's image processing also needs to be able to handle this format. The current default version of imagemagick installed on heroku:18 dynos is 6.8.9.9 and does not support processing heic image files. This [Heroku buildpack](http://devcenter.heroku.com/articles/buildpacks) vendors a version of ImageMagick with HEIF support binaries into your project. It is based on several resources including https://github.com/retailzipline/heroku-buildpack-imagemagick-heif.

This buildpack was created for `heroku-18` stacks.

The tar file in the [/build folder](./build) currently contains:

Version: ImageMagick 7.0.9-24 Q16 x86_64 2020-02-10 https://imagemagick.org
...
```
https://github.com/Rikku-1/heroku-buildpack-imagemagick-heif
```
...
```
heroku buildpacks:add https://github.com/Rikku-1/heroku-buildpack-imagemagick-heif --index 1 --app HEROKU_APP_NAME
```

## How to Build a New Binary

The binary in this repo was built in a heroku:18 docker image running in a local dev environment. The tar file was then copied into the `/build` directory in this repo and is used by the [compile script](./bin/compile).
...
1. Spin up a docker container with the heroku:18 stack. This will build and behave exactly the same way a heroku:18 dyno except you will have write access. (make sure docker is running on your machine). From command line:

     ```bash
     $ docker run --rm -it heroku/heroku:18-build
     ```
...
2. Get the libraries and dependencies you need(some of these already exist on the system):
     ...
     $ apt-get build-dep imagemagick libmagickcore-dev libde265 libheif webp libwebp-dev
     ```
...
3. Clone the libde265 and libheif libraries:
     ...
     $ wget http://deb.debian.org/debian/pool/main/libw/libwebp/libwebp_0.6.1.orig.tar.gz
     $ tar zxvf libwebp_0.6.1.orig.tar.gz
...
6. Install the libwebp-0.6.1 library:
...
6. Get, Configure and Install Newest Imagemagick:
    ...
    $ ./configure --with-heic=yes --with-webp=yes --prefix=/usr/src/imagemagick --without-gvc
    $ make && make install
    ```
